...
class AIAV(nn.Module):
    class Encoder(nn.Module):
        def __init__(self, seq_len, emb_dim):
            ...
            self.hidden = (torch.zeros(1, 1, self.emb_dim).to("cuda:0"), torch.zeros(1, 1, self.emb_dim).to("cuda:0"))
            self.model = nn.LSTM(
                input_size=self.inp,
    ...
    class Decoder(nn.Module):
        def __init__(self, seq_len, emb_dim):
            ...
            self.hidden = (torch.zeros(1, self.inp).to("cuda:0"), torch.zeros(1, self.emb_dim).to("cuda:0"))
            self.model = nn.LSTM(
                input_size=self.inp,
                hidden_size=self.emb_dim,
                proj_size=self.inp,
        ...
        def clear(self):
            self.hidden = (torch.zeros(1, self.inp).to("cuda:0"), torch.zeros(1, self.emb_dim).to("cuda:0"))
        def decode(self, x):
            out = torch.zeros(0,457).to(x.get_device())
            hidden, cells = self.hidden
